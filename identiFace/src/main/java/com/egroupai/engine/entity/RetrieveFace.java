...
public class RetrieveFace extends EngineFunc{
	...
	private boolean isShowThreadWindow;
	...
	public boolean isShowThreadWindow() {
		return isShowThreadWindow;
	}
	public void setShowThreadWindow(boolean isShowThreadWindow) {
		this.isShowThreadWindow = isShowThreadWindow;
	}	
	...
	public void generateCli() {
		...
		if(attributeCheck.stringsNotNull(ENGINEPATH,disk,trainedBinaryPath,trainedFaceInfoPath,JsonPath)){
			...
			cli = new StringBuilder(
					...
					+ (isShowThreadWindow==true?" --show-thread-window ":"")
